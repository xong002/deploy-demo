...
//import Joi from joi-browser to create schemas and validate entries
...
function SimpleForm() {
  ...
  */
  ...
  //This validate is done per input
  const validate = (event) => {
    ...
    const objToCompare = { [name]: value }; // use [] to get value of the name, otherwise the key will always be "name" string. typically object is {key:value}.
    const subSchema = { [name]: schema[name] }; //part of the schema we want to compare specifically

    //actual validation
    const result = Joi.validate(objToCompare, subSchema); // to compare the inputs with schema
